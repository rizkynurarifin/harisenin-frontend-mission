    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        try {
            ...
            const baseData = {
                ...
                casts: formData.casts ? formData.casts.split(",").map((c: string) => c.trim()) : [],
                creators: formData.creators ? formData.creators.split(",").map((c: string) => c.trim()) : [],
    ...
    return (
        <div className="p-4 md:p-10 lg:px-20 bg-other-page-header min-h-screen text-white font-lato">
            <div className="mb-8">
                ...
                <h1 className="text-2xl lg:text-3xl font-bold tracking-wide">Tambah Konten Baru</h1>
                <p className="text-secondary text-sm mt-1">Lengkapi informasi detail konten Chill kamu.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-[#222426] rounded-xl border border-white/5 shadow-2xl p-5 md:p-8 lg:p-10 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                    ...
                    <div className="grid grid-cols-3 gap-3 md:gap-4">
                        <div className="space-y-2">
                            ...
                            <input type="number" className="w-full bg-white/5 border border-white/10 rounded-lg p-3 outline-none text-sm" value={formData.year} onChange={(e) => setFormData({ ...formData, year: parseInt(e.target.value) })} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] md:text-xs font-bold text-secondary uppercase tracking-wider">Rating</label>
                            <input type="text" className="w-full bg-white/5 border border-white/10 rounded-lg p-3 outline-none text-sm" placeholder="4.5" value={formData.rating} onChange={(e) => setFormData({ ...formData, rating: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] md:text-xs font-bold text-secondary uppercase tracking-wider">Usia</label>
                            <select className="w-full bg-[#2d2f31] border border-white/10 rounded-lg p-3 outline-none cursor-pointer text-sm" value={formData.ageRating} onChange={(e) => setFormData({ ...formData, ageRating: e.target.value })}>
                ...
                <div className="space-y-2">
                    ...
                    <textarea rows={4} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 outline-none resize-none text-sm" placeholder="Tulis deskripsi singkat..." value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                </div>
